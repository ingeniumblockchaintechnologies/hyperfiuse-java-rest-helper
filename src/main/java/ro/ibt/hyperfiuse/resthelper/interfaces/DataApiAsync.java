...
public interface DataApiAsync {
	 ...
	 *                               not go as expected
	 */
	public ListenableFuture<Response> listRecords(String token, String entityName, boolean integrityCheck, boolean gZip,
			int skip, int limit) throws RestResponseException;
	 ...
	 * 
	 * @param token            User token
	 * @param recordPermission used to mention if the record can be accessed by
	 *                         everyone (PUBLIC), only by the users who have the
	 *                         permission to access it (PERMISSIONED), or only by
	 *                         the admin (PRIVATE). This parameter is not required.
	 ...
	 *                         permission to access it (PERMISSIONED), or only by
	 *                         the admin (PRIVATE). This parameter is not required.
	 * @param record           POJO record to be be used to update the specified
	 *                         event by record id
	 * @return
	 * @throws RestResponseException The exception which is thrown if something did
	 *                               not go as expected
